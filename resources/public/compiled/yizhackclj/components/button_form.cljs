(ns yizhackclj.components.button-form
  	(:require 
...
(defn button-form []
	...
	[:div.edit
		(when (and @state/edit-mode @state/selected-button)

			(let [	button 					(db/button 
												(:layer-id @state/selected-button)
												(:row @state/selected-button)
												(:column @state/selected-button)
											)
			  		...
			  		[:select 
						{
							...
							:on-change (fn [e]
								...
								(when (not= (-> e .-target .-value) 0)
									...
									(db/button-update button {:value (str "LN_" (-> e .-target .-value))})
								)
							)
						}  
